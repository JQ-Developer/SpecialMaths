...
def f(x):
  y = np.exp(x/4) + 2
  return y
def RiemannIzq(f, xi, xf, n):           #Argumentos: funcion (f), intervalo inicial (xi), intervalo final (xf), numero de particiones (n)
  x = np.linspace(xi, xf, n)            #Valores de 'x' en los 'n' intervalos, esta funcion devuelve un arreglo de numeros espaciados igualmente entre los intervalos xi,xf
...
A1, xbar, ybar = RiemannIzq(f, xi, xf, n1)        #Se llama la funcion y sus resultados se asignan a las variables 
...
print(tabulate(tabla, headers = ["Partición (n)", "Suma Riemann", "Error (%)"]))  #Se imprime el resultado de la tabla, ingresando como argumentos las cabeceras que llevara